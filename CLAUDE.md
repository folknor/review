...
- Codex `exec --json` routinely finishes the turn on disk (the rollout reaches `task_complete`) yet exits non-zero at shutdown and truncates *both* the `--json` stdout stream (no `turn.completed`, so `turns: 0`) and the `-o` backstop (`captured: false`). The signature is a split brain: stdout says `exit 1 / captured false / turns 0`, the rollout says `task_complete true`. When that happens `review` recovers the real report from the rollout's `final_answer` message rather than falling back to the last streamed `commentary` (an interim "I am now doing X" note that reads like a truncated death). If the rollout has no `final_answer` (the model ended on a tool call), the fallback text is `(codex completed the turn but emitted no final answer)` and the digest note flags it - distinct from a genuine hard freeze. Why codex exits 1 here is codex-side; `review` treats it as a non-event by reading the authoritative on-disk record.
