...
- `src/cli.rs` - Clap CLI. Archetype is an optional positional arg (omitted = bare: stdin sent unchanged); `init`, `config`, `sessions` and `incidents` are subcommands, and their names are reserved as archetype/group names (`config::RESERVED_NAMES`).
...
- `src/config_cmd.rs` - `review config [--json]`: the effective configuration from the same resolver a run uses, and outside a project the global layer alone (an orchestrator asks before it has picked a project) - files consulted, default providers and whether each provider is installed here, archetypes, groups, profiles with their source table and whatever they shadow. Env var *names* only. The JSON shape is a contract for orchestration procedures, which used to parse `.review.toml` themselves and would now get the wrong answer from it.
...
- Every run starts a fresh session - archetype priming prompt (if any) + stdin. Reviving a long-lived session on a cold cache reprocesses its ever-growing history; a fresh session costs ~one review's worth of tokens and can't act on stale accumulated context. The session is persistable and its ID is printed so follow-ups can go through `--session` while the cache is warm.
...
- Archetypes are optional and pure: `[archetypes]` name → prompt, no host/session binding. Without one a run is bare - stdin goes through unchanged, recorded under the archetype label `bare`. Overrides live in named profiles (`[<provider>.<profile>]` carrying model/effort/sandbox/env) selected with `--profile <name>`; `--profile` requires some layer to define it for every launched provider or the run errors naming every file searched. `sandbox` defaults to `read-only`, so a bare run can never modify files; a profile opts up to `workspace-write` (see [reference/sandbox.md](reference/sandbox.md)).
