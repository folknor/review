...
- `--session <id>` resumes a specific provider session and sends raw stdin — bypasses `.review.toml`, no PREFIX, no prime, no anchor. Requires a single `--provider`; mutually exclusive with `--oneshot` and `--anchor`. Validation of the session ID is delegated to the provider.
